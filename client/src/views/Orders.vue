    ...
    <div v-else>
      ...
      <div v-if="submittedOrders.length > 0" class="card submitted-card">
        <div class="card-header">
          <h3 class="card-title">{{ t('orders.submittedSection') }} ({{ submittedOrders.length }})</h3>
        </div>
...
export default {
  ...
  setup() {
    ...
    // Lead time = whole days between order_date and expected_delivery. Submitted
    // restocking orders carry an expected_delivery the backend computed from a
    // category-based lead time, so this just surfaces that to the user.
    const leadTimeDays = (order) => {
      if (!order.order_date || !order.expected_delivery) return 0
...
/* Highlight the submitted section so freshly-placed restocks are easy to spot. */
.submitted-card {
